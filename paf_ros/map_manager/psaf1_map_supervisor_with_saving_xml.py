...
class MapSupervisorCommonRoads(MapProvider):
    ...
    def update_world(self, world_info: CarlaWorldInfo):
        ...
        if self.map_name == world_info.map_name:
            ...
            for key in self.landmark_provider.available_categories():
                if 'Speed' in key:
                    rospy.loginfo(
                        "MapSupervisorCommonRoads: Adding " + key + " signs")
    ...
    def _gen_raw_scenario(self):
        ...
        if self.opendrive_loaded:
            ...
            opendrive = parse_opendrive(etree.parse(
    ...
    def _stop_signs_to_scenario(self, stops: list, neighbouring: bool):
        ...
        """
        for stop in stops:
            mapped_lanelets = self._find_traffic_light_lanelet(
                stop, self.max_angle_diff_sign, neighbouring=neighbouring)
            if len(mapped_lanelets) > 0:
                for lanelet in mapped_lanelets:
                    index = self._find_vertex_index(
                        lanelet, stop.pos_as_point())
                    self._add_sign_to_lanelet(
                        lanelet.lanelet_id, index, TrafficSignIDGermany.STOP)
            else:
    ...
    def _find_nearest_lanelet(self, goal: Point):
        ...
        while curr_radius < max_radius or nearest is not None:
            nearest = self.map_cr.lanelet_network.lanelets_in_proximity(
    ...
    def _find_traffic_light_lanelet(self, light: LandMarkPoint, max_angle_diff: float, neighbouring: bool = True):
        ...
        while not solution_found and curr_radius <= max_radius:
            nearest = self.map_cr.lanelet_network.lanelets_in_proximity(
                np.array([light.x, light.y]), curr_radius)
            if len(nearest) > 0:
                for i, lane in enumerate(nearest):
                    ...
                    lane_orientation = self._get_lanelet_orientation_to_light(
                        lane, use_end=True)
                    angle_diff = 180 - \
                        abs(abs(light.orientation - lane_orientation) - 180)
                    ...
                    if not is_intersection and angle_diff < max_angle_diff:
                        ...
                        dist_end = np.linalg.norm(
                            np.array([light.x, light.y]) - np.array(lane.center_vertices[-1]))
                        # get distance of the start point of that lanelet to the traffic light
                        dist_start = np.linalg.norm(
                            np.array([light.x, light.y]) - np.array(lane.center_vertices[0]))
        ...
        if solution_found:
            ...
            if neighbouring:
                best_lanelets.extend(self._get_all_fitting_neighbour_lanelets(
                    best_lanelets[0].lanelet_id))
        ...
        for lanelet in self.map_cr.lanelet_network.lanelets:
            if self.intersection[lanelet.lanelet_id]:
                ...
                while neighbour_lanelet.adj_right_same_direction and neighbour_lanelet.adj_right is not None:
                    neighbour_lanelet = self.map_cr.lanelet_network.find_lanelet_by_id(
                        neighbour_lanelet.adj_right)
                ...
                while neighbour_lanelet.adj_left_same_direction and neighbour_lanelet.adj_left is not None:
                    neighbour_lanelet = self.map_cr.lanelet_network.find_lanelet_by_id(
                        neighbour_lanelet.adj_left)
                    self.intersection[neighbour_lanelet.lanelet_id] = True

        # following the upper issue: now catch cases where you dont know your neighbour lanelet
        for lanelet in self.map_cr.lanelet_network.lanelets:
            if self.intersection[lanelet.lanelet_id]:
                center_point = lanelet.center_vertices[len(
                    lanelet.center_vertices) // 2]
                start_point = lanelet.center_vertices[0]
                end_point = lanelet.center_vertices[-1]
                length = np.linalg.norm(
                    np.array(end_point) - np.array(start_point))

                start_orientation = self._get_lanelet_orientation_to_light(
                    lanelet, use_end=False)
                end_orientation = self._get_lanelet_orientation_to_light(
                    lanelet, use_end=True)

                # get all lanelets nearby
                nearest = self.map_cr.lanelet_network.lanelets_in_proximity(
                    np.array(center_point), self.inter_width)
                for near_lanelet in nearest:
                    curr_start_point = near_lanelet.center_vertices[0]
                    curr_end_point = near_lanelet.center_vertices[-1]
                    curr_length = np.linalg.norm(
                        np.array(curr_end_point) - np.array(curr_start_point))

                    # neighbouring distance - should be less than half the intersection
                    neigh_dist = np.linalg.norm(
                        np.array(curr_end_point) - np.array(start_point))

                    curr_start_orientation = self._get_lanelet_orientation_to_light(
                        near_lanelet, use_end=False)
                    curr_end_orientation = self._get_lanelet_orientation_to_light(
                        near_lanelet, use_end=True)

                    # calculate angle_diff but consider that the two orientations should be opposite to each other
                    start_angle_diff = abs(
                        abs(start_orientation - curr_end_orientation) - 180)
                    end_angle_diff = abs(
                        abs(end_orientation - curr_start_orientation) - 180)
    ...
    def _traffic_lights_to_scenario(self, lights: list):
        ...
        for light in lights:
            mapped_lanelets = self._find_traffic_light_lanelet(
                light, self.max_angle_diff_light)
            if len(mapped_lanelets) > 0:
                for lanelet in mapped_lanelets:
    ...
    def _add_light_to_lanelet(self, lanelet_id: int):
        ...
        """
        self.cur_mark_id += 1
        pos = self.map_cr.lanelet_network.find_lanelet_by_id(
            lanelet_id).center_vertices[-1]
        traffic_light = TrafficLight(self.cur_mark_id, [], pos)
        id_set = set()
        id_set.add(lanelet_id)
        self.map_cr.lanelet_network.add_traffic_light(
            traffic_light, lanelet_ids=deepcopy(id_set))
    ...
    def _add_sign_to_lanelet(self, lanelet_id: int, pos_index: int, typ: TrafficSignIDGermany, additional: list = []):
        ...
        self.map_cr.lanelet_network.add_traffic_sign(
    ...
    def _get_lanelet_orientation_to_light(lanelet: Lanelet, use_end: bool = True):
        ...
        euler_angle_yaw = math.degrees(math.atan2(
            rel_y * abs(pos[1] - prev_pos[1]), rel_x * abs(pos[0] - prev_pos[0])))
    ...
    def _generate_dummy_planning_problem() -> PlanningProblem:
        ...
        goal_state: State = State(
            position=circle, time_step=Interval(0, 10000.0))
