...
class RoadNetworkToolbox(QDockWidget):
    ...
    def connect_gui_elements(self):
        ...
        self.road_network_toolbox.button_add_traffic_sign.clicked.connect(lambda: self.add_traffic_sign())
        ...
        self.road_network_toolbox.button_create_lanelet_from_vertices.clicked.connect(
            lambda: self.create_lanelet_from_vertices()
        )
    ...
    def add_lanelet(
        ...
        adjacent_left = (
            int(self.road_network_toolbox.adjacent_left.currentText())
            if self.road_network_toolbox.adjacent_left.currentText() != "None"
            else None
        )
        adjacent_right = (
            int(self.road_network_toolbox.adjacent_right.currentText())
        ...
        lanelet_type = {
            LaneletType(ty) for ty in self.road_network_toolbox.lanelet_type.get_checked_items() if ty != "None"
        }
        user_one_way = {
            RoadUser(user) for user in self.road_network_toolbox.road_user_oneway.get_checked_items() if user != "None"
        }
        user_bidirectional = {
            RoadUser(user)
            for user in self.road_network_toolbox.road_user_bidirectional.get_checked_items()
            if user != "None"
        }
        ...
        if (
            self.road_network_toolbox.stop_line_start_x.text() != ""
            ...
            if connect_to_last_selection:
                if self.last_added_lanelet_id is not None:
                    MapCreator.fit_to_predecessor(
                        self.current_scenario.lanelet_network.find_lanelet_by_id(self.last_added_lanelet_id), lanelet
                    )
                else:
    ...
    def update_lanelet(self):
        ...
        self.add_lanelet(
            selected_lanelet.lanelet_id, True, selected_lanelet.left_vertices, selected_lanelet.right_vertices
        )
    ...
    def update_lanelet_information(self, lanelet: Lanelet = None):
        ...
        self.road_network_toolbox.road_user_oneway.set_checked_items([str(user.value) for user in lanelet.user_one_way])
        ...
        if lanelet.stop_line is not None:
            ...
            self.road_network_toolbox.line_marking_stop_line.setCurrentText(str(lanelet.stop_line.line_marking.value))
        else:
        ...
        if self.road_network_toolbox.selected_traffic_sign.currentText() not in ["", "None"]:
            ...
            for elem in traffic_sign.traffic_sign_elements:
                ...
                if len(elem.additional_values) > 0:
                    ...
                    self.road_network_toolbox.traffic_sign_element_table.setItem(num_rows - 1, 1, QTableWidgetItem(""))
        else:
    ...
    def add_traffic_light(self, traffic_light_id: int = None):
        ...
        traffic_light_direction = TrafficLightDirection(
            self.road_network_toolbox.traffic_light_directions.currentText()
        )
    ...
    def create_traffic_lights(self):
        ...
        lanelet_ids = [
            int(lanelet_id)
            for lanelet_id in self.road_network_toolbox.referenced_lanelets_traffic_light.get_checked_items()
        ]
        if not lanelet_ids:
            return
        self.road_network_toolbox.referenced_lanelets_traffic_light.clear()
    ...
    def update_left_of_combobox(self, incoming_ids: List[str] = None):
        ...
        if incoming_ids is None:
            incoming_ids = [
                self.road_network_toolbox.intersection_incomings_table.item(row, 0).text()
    ...
    def add_intersection(self, intersection_id: int = None):
        ...
        for row in range(self.road_network_toolbox.intersection_incomings_table.rowCount()):
            ...
            incoming_lanelets = {
                int(item)
                for item in self.road_network_toolbox.intersection_incomings_table.cellWidget(
                    row, 1
                ).get_checked_items()
            }
            ...
            if len(successor_left) + len(successor_right) + len(successor_straight) < 1:
                ...
                return
            left_of = (
                int(self.road_network_toolbox.intersection_incomings_table.cellWidget(row, 5).currentText())
    ...
    def update_intersection_information(self):
        ...
        if self.road_network_toolbox.selected_intersection.currentText() not in ["", "None"]:
            ...
            for incoming in intersection.incomings:
                ...
                self.road_network_toolbox.intersection_incomings_table.cellWidget(num_rows - 1, 1).set_checked_items(
                    [str(la_id) for la_id in incoming.incoming_lanelets]
                )
                self.road_network_toolbox.intersection_incomings_table.cellWidget(num_rows - 1, 2).set_checked_items(
                    [str(la_id) for la_id in incoming.successors_left]
                )
                self.road_network_toolbox.intersection_incomings_table.cellWidget(num_rows - 1, 3).set_checked_items(
                    [str(la_id) for la_id in incoming.successors_straight]
                )
                self.road_network_toolbox.intersection_incomings_table.cellWidget(num_rows - 1, 4).set_checked_items(
                    [str(la_id) for la_id in incoming.successors_right]
                )
                index = self.road_network_toolbox.intersection_incomings_table.cellWidget(num_rows - 1, 5).findText(
                    str(incoming.left_of)
                )
                self.road_network_toolbox.intersection_incomings_table.cellWidget(num_rows - 1, 5).setCurrentIndex(
                    index
                )
    ...
    def connect_lanelets(self):
        ...
        connected_lanelet = MapCreator.connect_lanelets(
            selected_lanelet_one, selected_lanelet_two, self._get_new_lanelet_id()
        )
    ...
    def optimize_vertices(self):
        ...
        for i in range(3, len(self.center_vertices)):
            ...
            v1x = (
                pow((1.0 - t1), 3) * p1[0]
                + 3 * t1 * pow((1.0 - t1), 2) * p2[0]
                + 3 * pow(t1, 2) * (1.0 - t1) * p3[0]
                + pow(t1, 3) * p4[0]
            )
            v1y = (
                pow((1.0 - t1), 3) * p1[1]
                + 3 * t1 * pow((1.0 - t1), 2) * p2[1]
                + 3 * pow(t1, 2) * (1.0 - t1) * p3[1]
                + pow(t1, 3) * p4[1]
            )
            v2x = (
                pow((1.0 - t2), 3) * p1[0]
                + 3 * t2 * pow((1.0 - t2), 2) * p2[0]
                + 3 * pow(t2, 2) * (1.0 - t2) * p3[0]
                + pow(t2, 3) * p4[0]
            )
            v2y = (
                pow((1.0 - t2), 3) * p1[1]
                + 3 * t2 * pow((1.0 - t2), 2) * p2[1]
                + 3 * pow(t2, 2) * (1.0 - t2) * p3[1]
                + pow(t2, 3) * p4[1]
            )
    ...
    def create_lanelet_from_vertices(self):
        ...
        for i in range(0, num_vertices):
            self.center_vertices.append(
                [
                    float(self.road_network_toolbox.center_vertices_table.cellWidget(i, 0).text()),
                    float(self.road_network_toolbox.center_vertices_table.cellWidget(i, 1).text()),
                ]
            )
    ...
    def _split_lanelet_internal(self, lanelet_id: int, split_pos_x: float, split_pos_y: float) -> Tuple[int, int]:
        ...
        if sep_index > 1 and (len(lanelet_center_list) - 1) - sep_index >= 1:
            ...
            additional = np.array(
                [
                    [left_1[-2][0] + factor_x, left_1[-2][1] + factor_y],
            ...
            additional = np.array(
                [
                    [center_1[-2][0] + factor_x, center_1[-2][1] + factor_y],
            ...
            additional = np.array(
                [
                    [right_1[-2][0] + factor_x, right_1[-2][1] + factor_y],
            ...
            additional = np.array(
                [
                    [left_2[-2][0] + factor_x, left_2[-2][1] + factor_y],
            ...
            additional = np.array(
                [
                    [center_2[-2][0] + factor_x, center_2[-2][1] + factor_y],
            ...
            additional = np.array(
                [
                    [right_2[-2][0] + factor_x, right_2[-2][1] + factor_y],
            ...
            # create new lanelets:
            lanelet_1 = Lanelet(
                lanelet_id=id_lane_1,
                predecessor=lanelet_copy.predecessor,
                left_vertices=left_1,
                center_vertices=center_1,
                right_vertices=right_1,
                successor=[id_lane_2],
                adjacent_left=lanelet_copy.adj_left,
            ...
            lanelet_2 = Lanelet(
                lanelet_id=id_lane_2,
                predecessor=[id_lane_1],
                left_vertices=left_2,
                center_vertices=center_2,
                right_vertices=right_2,
                successor=lanelet_copy.successor,
                adjacent_left=lanelet_copy.adj_left,
                adjacent_right=lanelet_copy.adj_right,
                adjacent_right_same_direction=lanelet_copy.adj_right_same_direction,
                adjacent_left_same_direction=lanelet_copy.adj_left_same_direction,
    ...
    def _split_lanelet(self, matching_lanelet_id: int, split_point_x: float, split_point_y: float):
        ...
        if self.current_scenario.lanelet_network.find_lanelet_by_id(matching_lanelet_id) is None:
            return None, None
        right_same_dir = self.current_scenario.lanelet_network.find_lanelet_by_id(
            matching_lanelet_id
        ).adj_right_same_direction
        left_same_dir = self.current_scenario.lanelet_network.find_lanelet_by_id(
            matching_lanelet_id
        ...
        if right_1 is not None and right_2 is not None:
            ...
            self.current_scenario.lanelet_network.find_lanelet_by_id(
                matching_1
            ).adj_right_same_direction = right_same_dir
            self.current_scenario.lanelet_network.find_lanelet_by_id(
                matching_2
            ).adj_right_same_direction = right_same_dir
            next = None
            ...
            if right_same_dir:
                ...
                self.current_scenario.lanelet_network.find_lanelet_by_id(
                    right_1
                ).adj_left_same_direction = right_same_dir
                self.current_scenario.lanelet_network.find_lanelet_by_id(
                    right_2
                ).adj_left_same_direction = right_same_dir
                next_dir = True
            ...
            while next is not None:
                ...
                if next_dir:
                    ...
                    next_dir = self.current_scenario.lanelet_network.find_lanelet_by_id(
                        right_1_old
                    ...
                    next_dir = not self.current_scenario.lanelet_network.find_lanelet_by_id(
                        right_1_old
        ...
        if left_1 is not None and left_2 is not None:
            ...
            if left_same_dir:
                ...
                self.current_scenario.lanelet_network.find_lanelet_by_id(
                    left_1
                ).adj_right_same_direction = left_same_dir
                self.current_scenario.lanelet_network.find_lanelet_by_id(
                    left_2
                ).adj_right_same_direction = left_same_dir
                next_dir = True
