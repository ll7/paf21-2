...
# >>>>>>> develop
    ...
    def create_lanelet_editor_widget(self):
        ...
        label_split_lanelets = QLabel("Split Lanelets")
        label_split_lanelets.setFont(QFont("Arial", 11, QFont.Bold))

        self.split_lanelets_info_click = QCheckBox("Get Lanelet and Split Position Per Click")
        self.split_lanelets_neighbours = QCheckBox("Don't Split Adjacent Lanelets")

        self.x_position_split = QLineEdit()
        self.x_position_split.setValidator(QDoubleValidator())
        self.x_position_split.setMaxLength(8)
        self.x_position_split.setAlignment(Qt.AlignRight)

        self.y_position_split = QLineEdit()
        self.y_position_split.setValidator(QDoubleValidator())
        self.y_position_split.setMaxLength(8)
        self.y_position_split.setAlignment(Qt.AlignRight)

        self.button_split_selected_lanelet = QPushButton("Split Selected Lanelet")

        self.selected_lanelet_split = QComboBox()

        split_lanelets_layout = QFormLayout()
        split_lanelets_groupbox = QGroupBox()
        split_lanelets_groupbox.setLayout(split_lanelets_layout)
        split_lanelets_layout.addRow(label_split_lanelets)
        split_lanelets_layout.addRow(self.split_lanelets_info_click)
        split_lanelets_layout.addRow(self.split_lanelets_neighbours)
        split_lanelets_layout.addRow("X-Position:", self.x_position_split)
        split_lanelets_layout.addRow("Y-Position:", self.y_position_split)
        split_lanelets_layout.addRow("Selected Lanelet:", self.selected_lanelet_split)
